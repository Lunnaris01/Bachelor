...
class ACERNetwork(object):
    ...
    def __init__(self, session, env, scope="global", lr=2e-4, decay=0.99, eps=1e-6,
        ...
        with tf.name_scope("update"):
            ...
            grads_vars = list(zip(clipped_grads, self.tf_trainable))
    ...
    def get_retrace_values(self, states, actions):
        ...
        vals = self.sess.run([self.pi, self.action_quality, self.values],
                             {self.tf_states: states, self.tf_actions: actions, self.tf_states: states})
        return vals
    ...
    def update_step(self, states, actions, targets, importance_w, summary=False):
        ...
        if summary:
            _, summary, step = self.sess.run([self.train_op, self.summary_op, self.global_step], feeddict)
            return summary, step

        _, step = self.sess.run([self.train_op, self.global_step], feeddict)
        return None, step
